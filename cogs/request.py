...
class request(commands.Cog):
    ...
    async def 요청(self, ctx, args):
        if ctx.author.id in self.ignore_id:
            ...
            embed1.add_field(name='요청 내용', value=args, inline=True)
            embed1.set_thumbnail(url=ctx.author.avatar_url)
            await botch.send(f"<@777496131177414656>")
            await botch.send(embed=embed1)

            embed2 = discord.Embed(title='[ 성공적으로 요청이 전송됐습니다 ]', description=f"요청 내용: {args[:]}", color=0x62c1cc)
